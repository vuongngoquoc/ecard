...
*/	
	...
	if($what=="delete_account"){
		...
		$show_info ="$member_display_inactive_acc_message_member_account_updated<br />";
		$what="";
		set_global_var("what","");
	}
	...
	$begin_6month_from_today=$begin_today_timestamp-(86400*30*6);
	$total_member=get_dbvalue("max_ecuser","count(user_id)","user_member1<>'0' and user_lastlogin < $begin_6month_from_today");
	set_global_var("total_member",$total_member);
	
	if($cmd_button =="List"){
		if($list_item == "all"){
			$list_data =set_array_from_query("max_ecuser","*","user_member1<>'0' and user_lastlogin < $begin_6month_from_today Order by user_name_id LIMIT $start,$row_per_page");
			$count_list=get_dbvalue("max_ecuser","count(user_id)","user_member1<>'0' and user_lastlogin < $begin_6month_from_today");
			$member_display_inactive_acc_txt_all_members_not_login_6_months=str_replace("%total_member%","$total_member",$member_display_inactive_acc_txt_all_members_not_login_6_months);
			$show_info ="$member_display_inactive_acc_txt_all_members_not_login_6_months";
		}
		elseif($list_item == "all_DESC"){
			$list_data =set_array_from_query("max_ecuser","*","user_member1<>'0' and user_lastlogin < $begin_6month_from_today Order by user_name_id DESC LIMIT $start,$row_per_page");
			$count_list=get_dbvalue("max_ecuser","count(user_id)","user_member1<>'0' and user_lastlogin < $begin_6month_from_today");
			$member_display_inactive_acc_txt_all_members_not_login_6_months=str_replace("%total_member%","$total_member",$member_display_inactive_acc_txt_all_members_not_login_6_months);
			$show_info ="$member_display_inactive_acc_txt_all_members_not_login_6_months";
		}
		elseif($list_item == "all2"){
			$list_data =set_array_from_query("max_ecuser","*","user_member1<>'0' and user_lastlogin < $begin_6month_from_today Order by user_mg_id LIMIT $start,$row_per_page");
			$count_list=get_dbvalue("max_ecuser","count(user_id)","user_member1<>'0' and user_lastlogin < $begin_6month_from_today");
			$member_display_inactive_acc_txt_all_members_not_login_6_months=str_replace("%total_member%","$total_member",$member_display_inactive_acc_txt_all_members_not_login_6_months);
			$show_info ="$member_display_inactive_acc_txt_all_members_not_login_6_months";
		}
		elseif($list_item == "all2_DESC"){
			$list_data =set_array_from_query("max_ecuser","*","user_member1<>'0' and user_lastlogin < $begin_6month_from_today Order by user_mg_id DESC LIMIT $start,$row_per_page");
			$count_list=get_dbvalue("max_ecuser","count(user_id)","user_member1<>'0' and user_lastlogin < $begin_6month_from_today");
			$member_display_inactive_acc_txt_all_members_not_login_6_months=str_replace("%total_member%","$total_member",$member_display_inactive_acc_txt_all_members_not_login_6_months);
			$show_info ="$member_display_inactive_acc_txt_all_members_not_login_6_months";
		}		
	...
	elseif($cmd_button =="View"){
		...
		$list_data =set_array_from_query("max_ecuser","*","user_member1<>'0' and user_lastlogin < $begin_time_before Order by user_name_id DESC LIMIT $start,$row_per_page");
		$count_list=get_dbvalue("max_ecuser","count(user_id)","user_member1<>'0' and user_lastlogin < $begin_time_before");
		$member_display_inactive_acc_txt_all_members_not_login_before_today=str_replace("%num_day%","$num_day",$member_display_inactive_acc_txt_all_members_not_login_before_today);
		$member_display_inactive_acc_txt_all_members_not_login_before_today=str_replace("%num_what%","$num_what",$member_display_inactive_acc_txt_all_members_not_login_before_today);
		$member_display_inactive_acc_txt_all_members_not_login_before_today=str_replace("%count_list%","$count_list",$member_display_inactive_acc_txt_all_members_not_login_before_today);
		$show_info ="$member_display_inactive_acc_txt_all_members_not_login_before_today";
	}	
	else{
		$list_data =set_array_from_query("max_ecuser","*","user_member1<>'0' and user_lastlogin < $begin_6month_from_today Order by user_name_id LIMIT $start,$row_per_page");
		$count_list=get_dbvalue("max_ecuser","count(user_id)","user_member1<>'0' and user_lastlogin < $begin_6month_from_today");
	...
	for ($z=$start; $z<$end; $z++) {
		...
		if($array_country_info[ip_country2]){
			$ip2country="<br />$member_display_inactive_acc_txt_ip2country: <img border=\"0\" src=\"$ecard_url/resource/flags/$array_country_info[ip_country2].gif\" alt=\"$array_country_info[ip_country]\" title=\"$array_country_info[ip_country]\" /> <strong>$array_country_info[ip_country_name]</strong>";
		}
		else{
			$ip2country="";
		}
		...
		if($row_data[user_beclosed] == 1){
			$member_display_inactive_acc_txt_account_will_be_suspended=str_replace("%date_end_acct%","$date_end_acct",$member_display_inactive_acc_txt_account_will_be_suspended);
			$show_account_status = "$member_display_inactive_acc_txt_account_will_be_suspended";
			//Show Undo button
			$show_delete_button="<img onclick=\"HideItAll();document.getElementById('cell$val').style.backgroundColor='#FAEDC8';document.getElementById('cell$val').style.border='thick solid #FCAA03';if(window.confirm('$member_display_inactive_acc_message_confirm_to_suspend_delete_account')){location.href='index.php?step=$step&what=undo_delete&user_id=$val&page=$page&row_number=$row_number&cmd_button=$cmd_button&list_item=$list_item&search_field=$search_field&keyword=$keyword&num_day=$num_day&num_what=$num_what&from_month=$from_month&from_day=$from_day&from_year=$from_year&to_day=$to_day&to_month=$to_month&to_year=$to_year';}else{HideItAll();}\" border=\"0\" src=\"html/07_icon_undo_delete.gif\" style=\"cursor:pointer\" alt=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" title=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" />";
		}
		elseif($row_data[user_beclosed] == 2){
			$member_display_inactive_acc_txt_account_will_be_deleted=str_replace("%date_end_acct%","$date_end_acct",$member_display_inactive_acc_txt_account_will_be_deleted);
			$show_account_status ="$member_display_inactive_acc_txt_account_will_be_deleted";
			//Show Undo button
			$show_delete_button="<img onclick=\"HideItAll();document.getElementById('cell$val').style.backgroundColor='#FAEDC8';document.getElementById('cell$val').style.border='thick solid #FCAA03';if(window.confirm('$member_display_inactive_acc_message_confirm_to_suspend_delete_account')){location.href='index.php?step=$step&what=undo_delete&user_id=$val&page=$page&row_number=$row_number&cmd_button=$cmd_button&list_item=$list_item&search_field=$search_field&keyword=$keyword&num_day=$num_day&num_what=$num_what&from_month=$from_month&from_day=$from_day&from_year=$from_year&to_day=$to_day&to_month=$to_month&to_year=$to_year';}else{HideItAll();}\" border=\"0\" src=\"html/07_icon_undo_delete.gif\" style=\"cursor:pointer\" alt=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" title=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" />";
		}
		elseif($row_data[user_beclosed] == 3){
			$member_display_inactive_acc_txt_account_will_be_downgraded=str_replace("%date_end_acct%","$date_end_acct",$member_display_inactive_acc_txt_account_will_be_downgraded);
			$show_account_status ="$member_display_inactive_acc_txt_account_will_be_downgraded";
			//Show Undo button
			$show_delete_button="<img onclick=\"HideItAll();document.getElementById('cell$val').style.backgroundColor='#FAEDC8';document.getElementById('cell$val').style.border='thick solid #FCAA03';if(window.confirm('$member_display_inactive_acc_message_confirm_to_suspend_delete_account')){location.href='index.php?step=$step&what=undo_delete&user_id=$val&page=$page&row_number=$row_number&cmd_button=$cmd_button&list_item=$list_item&search_field=$search_field&keyword=$keyword&num_day=$num_day&num_what=$num_what&from_month=$from_month&from_day=$from_day&from_year=$from_year&to_day=$to_day&to_month=$to_month&to_year=$to_year';}else{HideItAll();}\" border=\"0\" src=\"html/07_icon_undo_delete.gif\" style=\"cursor:pointer\" alt=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" title=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" />";
		}
		else{
		...
		foreach($row_data as $key=>$value){
			...
			if($row_data[user_gender] == 0){
				$show_user_gender ="$member_display_inactive_acc_txt_male";
			}
			else{
				$show_user_gender ="$member_display_inactive_acc_txt_female";
			}
			set_global_var("show_user_gender",$show_user_gender);

			if($row_data[user_marital] == 0){
				$show_user_marital ="$member_display_inactive_acc_txt_single";
			}
			elseif($row_data[user_marital] == 1){
				$show_user_marital ="$member_display_inactive_acc_txt_marriage";
			}
			elseif($row_data[user_marital] == 2){
				$show_user_marital ="$member_display_inactive_acc_txt_divorced";
			}
			elseif($row_data[user_marital] == 3){
				$show_user_marital ="$member_display_inactive_acc_txt_widowed";
			}
			...
			if(count($list_payment)>0){
				...
				foreach($list_payment as $row_data_order){
					...
					if($row_data_order[pay_what]=="0"){
						$show_type="$member_display_inactive_acc_txt_paypercard";
					}
					else{
						$show_type="$member_display_inactive_acc_txt_upgrade_acc";
					}
		...
		$show_list_table .="<td width=\"40%\" style=\"padding:4px;cursor:pointer;\" title=\"$member_display_inactive_acc_tooltip_click_to_view_detail\" id=\"cell$val\" onclick=\"HideItAll();this.style.backgroundColor='#FAEDC8';this.style.border='thick solid #FCAA03';ShowDivCenterPage('popup_detail$val')\">$member_display_inactive_acc_txt_username_id: <span style=\"color:green\">$row_data[user_name_id]</span>$ip2country</td>\n";
		...
		if($row_data[user_member1]!="0" && $row_data[user_member1]!=""){
			...
			if($list_acct_sub[user_id]!=""){
				...
				if($array_country_infosub[ip_country2]){
					$ip2countrysub="<br /><img src=\"html/07_icon_upper.gif\" alt=\"$member_display_inactive_acc_tooltip_sub_account\"/> $member_display_inactive_acc_txt_ip2country: <img border=\"0\" src=\"$ecard_url/resource/flags/$array_country_infosub[ip_country2].gif\" alt=\"$array_country_infosub[ip_country]\" title=\"$array_country_infosub[ip_country]\" /> <strong>$array_country_infosub[ip_country_name]</strong>";
				}
				else{
					$ip2countrysub="";
				}
				...
				if($list_acct_sub[user_beclosed] == 1){
					$show_account_status_sub = "$date_end_acct_sub<br />(will be suspended)";
					//Show Undo button
					$show_delete_button_sub="<img onclick=\"HideItAll();document.getElementById('cell$list_acct_sub[user_id]').style.backgroundColor='#FAEDC8';document.getElementById('cell$list_acct_sub[user_id]').style.border='thick solid #FCAA03';if(window.confirm('$member_display_inactive_acc_message_confirm_to_suspend_delete_account')){location.href='index.php?step=$step&what=undo_delete&user_id=$list_acct_sub[user_id]&page=$page&row_number=$row_number&cmd_button=$cmd_button&list_item=$list_item&search_field=$search_field&keyword=$keyword&num_day=$num_day&num_what=$num_what&from_month=$from_month&from_day=$from_day&from_year=$from_year&to_day=$to_day&to_month=$to_month&to_year=$to_year';}else{HideItAll();}\" border=\"0\" src=\"html/07_icon_undo_delete.gif\" style=\"cursor:pointer\" alt=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" title=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" />";
				}
				elseif($list_acct_sub[user_beclosed] == 2){
					$show_account_status_sub ="$date_end_acct_sub<br />(will be deleted)";
					//Show Undo button
					$show_delete_button_sub="<img onclick=\"HideItAll();document.getElementById('cell$list_acct_sub[user_id]').style.backgroundColor='#FAEDC8';document.getElementById('cell$list_acct_sub[user_id]').style.border='thick solid #FCAA03';if(window.confirm('$member_display_inactive_acc_message_confirm_to_suspend_delete_account')){location.href='index.php?step=$step&what=undo_delete&user_id=$list_acct_sub[user_id]&page=$page&row_number=$row_number&cmd_button=$cmd_button&list_item=$list_item&search_field=$search_field&keyword=$keyword&num_day=$num_day&num_what=$num_what&from_month=$from_month&from_day=$from_day&from_year=$from_year&to_day=$to_day&to_month=$to_month&to_year=$to_year';}else{HideItAll();}\" border=\"0\" src=\"html/07_icon_undo_delete.gif\" style=\"cursor:pointer\" alt=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" title=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" />";
				}
				elseif($list_acct_sub[user_beclosed] == 3){
					$show_account_status_sub ="$date_end_acct_sub<br />(will be down grade to free basic acct)";
					//Show Undo button
					$show_delete_button_sub="<img onclick=\"HideItAll();document.getElementById('cell$list_acct_sub[user_id]').style.backgroundColor='#FAEDC8';document.getElementById('cell$list_acct_sub[user_id]').style.border='thick solid #FCAA03';if(window.confirm('$member_display_inactive_acc_message_confirm_to_suspend_delete_account')){location.href='index.php?step=$step&what=undo_delete&user_id=$list_acct_sub[user_id]&page=$page&row_number=$row_number&cmd_button=$cmd_button&list_item=$list_item&search_field=$search_field&keyword=$keyword&num_day=$num_day&num_what=$num_what&from_month=$from_month&from_day=$from_day&from_year=$from_year&to_day=$to_day&to_month=$to_month&to_year=$to_year';}else{HideItAll();}\" border=\"0\" src=\"html/07_icon_undo_delete.gif\" style=\"cursor:pointer\" alt=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" title=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" />";
				}
				else{
				...
				$show_list_table .="<td width=\"40%\" style=\"padding:4px;cursor:pointer;\" title=\"$member_display_inactive_acc_tooltip_click_to_view_detail\" id=\"cell$list_acct_sub[user_id]\" onclick=\"HideItAll();this.style.backgroundColor='#FAEDC8';this.style.border='thick solid #FCAA03';ShowDivCenterPage('popup_detail$list_acct_sub[user_id]')\"><img src=\"html/07_icon_upper.gif\" alt=\"$member_display_inactive_acc_tooltip_sub_account\"/> $member_display_inactive_acc_txt_username_id: <span style=\"color:green\">$row_data[user_member1]</span>$ip2countrysub</td>\n";
		...
		if($row_data[user_member2]!="0" && $row_data[user_member2]!=""){
			...
			if($list_acct_sub[user_id]!=""){
				...
				$date_end_acct_sub=DateFormat($list_acct_sub[user_dateclose]);
				if($list_acct_sub[user_beclosed] == 1){
					$member_display_inactive_acc_txt_account_will_be_suspended=str_replace("%date_end_acct%","$date_end_acct_sub",$member_display_inactive_acc_txt_account_will_be_suspended);
					$show_account_status_sub = "$member_display_inactive_acc_txt_account_will_be_suspended";
					//Show Undo button
					$show_delete_button_sub="<img onclick=\"HideItAll();document.getElementById('cell$list_acct_sub[user_id]').style.backgroundColor='#FAEDC8';document.getElementById('cell$list_acct_sub[user_id]').style.border='thick solid #FCAA03';if(window.confirm('$member_display_inactive_acc_message_confirm_to_suspend_delete_account')){location.href='index.php?step=$step&what=undo_delete&user_id=$list_acct_sub[user_id]&page=$page&row_number=$row_number&cmd_button=$cmd_button&list_item=$list_item&search_field=$search_field&keyword=$keyword&num_day=$num_day&num_what=$num_what&from_month=$from_month&from_day=$from_day&from_year=$from_year&to_day=$to_day&to_month=$to_month&to_year=$to_year';}else{HideItAll();}\" border=\"0\" src=\"html/07_icon_undo_delete.gif\" style=\"cursor:pointer\" alt=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" title=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" />";
				}
				elseif($list_acct_sub[user_beclosed] == 2){
					$member_display_inactive_acc_txt_account_will_be_deleted=str_replace("%date_end_acct%","$date_end_acct_sub",$member_display_inactive_acc_txt_account_will_be_deleted);
					$show_account_status_sub ="$member_display_inactive_acc_txt_account_will_be_deleted";
					//Show Undo button
					$show_delete_button_sub="<img onclick=\"HideItAll();document.getElementById('cell$list_acct_sub[user_id]').style.backgroundColor='#FAEDC8';document.getElementById('cell$list_acct_sub[user_id]').style.border='thick solid #FCAA03';if(window.confirm('$member_display_inactive_acc_message_confirm_to_suspend_delete_account')){location.href='index.php?step=$step&what=undo_delete&user_id=$list_acct_sub[user_id]&page=$page&row_number=$row_number&cmd_button=$cmd_button&list_item=$list_item&search_field=$search_field&keyword=$keyword&num_day=$num_day&num_what=$num_what&from_month=$from_month&from_day=$from_day&from_year=$from_year&to_day=$to_day&to_month=$to_month&to_year=$to_year';}else{HideItAll();}\" border=\"0\" src=\"html/07_icon_undo_delete.gif\" style=\"cursor:pointer\" alt=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" title=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" />";
				}
				elseif($list_acct_sub[user_beclosed] == 3){
					$member_display_inactive_acc_txt_account_will_be_downgraded=str_replace("%date_end_acct%","$date_end_acct_sub",$member_display_inactive_acc_txt_account_will_be_downgraded);
					$show_account_status_sub ="$member_display_inactive_acc_txt_account_will_be_downgraded";
					//Show Undo button
					$show_delete_button_sub="<img onclick=\"HideItAll();document.getElementById('cell$list_acct_sub[user_id]').style.backgroundColor='#FAEDC8';document.getElementById('cell$list_acct_sub[user_id]').style.border='thick solid #FCAA03';if(window.confirm('$member_display_inactive_acc_message_confirm_to_suspend_delete_account')){location.href='index.php?step=$step&what=undo_delete&user_id=$list_acct_sub[user_id]&page=$page&row_number=$row_number&cmd_button=$cmd_button&list_item=$list_item&search_field=$search_field&keyword=$keyword&num_day=$num_day&num_what=$num_what&from_month=$from_month&from_day=$from_day&from_year=$from_year&to_day=$to_day&to_month=$to_month&to_year=$to_year';}else{HideItAll();}\" border=\"0\" src=\"html/07_icon_undo_delete.gif\" style=\"cursor:pointer\" alt=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" title=\"$member_display_inactive_acc_tooltip_undo_suspend_delete_account\" />";
				}
				else{
				...
				$show_list_table .="<tr id=\"tr$list_acct_sub[user_id]\" style=\"background-color: lightyellow;line-height:20px\">\n";
				$show_list_table .="<td width=\"40%\" style=\"padding:4px;cursor:pointer;\" title=\"$member_display_inactive_acc_tooltip_click_to_view_detail\" id=\"cell$list_acct_sub[user_id]\" onclick=\"HideItAll();this.style.backgroundColor='#FAEDC8';this.style.border='thick solid #FCAA03';ShowDivCenterPage('popup_detail$list_acct_sub[user_id]')\"><img src=\"html/07_icon_upper.gif\" alt=\"$member_display_inactive_acc_tooltip_sub_account\"/> $member_display_inactive_acc_txt_username_id: <span style=\"color:green\">$row_data[user_member2]</span>$ip2countrysub</td>\n";
				$show_list_table .="<td width=\"30%\" align=\"center\">$show_box_membergroup_sub</td>\n";
				$show_list_table .="<td width=\"28%\" align=\"center\">$show_account_status_sub</td>\n";
				$show_list_table .="<td width=\"1%\" align=\"center\">$show_delete_button_sub</td>\n";
				$show_list_table .="<td width=\"1%\" align=\"center\"><input onclick=\"if(this.checked){document.getElementById('tr$list_acct_sub[user_id]').style.backgroundColor='#E4E4D3';}else{document.getElementById('tr$list_acct_sub[user_id]').style.backgroundColor='#EAEAEA';}\" type=\"checkbox\" id=\"bk$list_acct_sub[user_id]\" name=\"mylist_id$list_acct_sub[user_id]\" value=\"$list_acct_sub[user_id]\" /></td>\n";
	...
	$show_member_group_all_user="<select id=\"mg_id_all\" name=\"mg_id_all\" size=\"1\" style=\"width:80%\"><option value=\"\">Click here to select member group name</option>\n";
	foreach($list_mg as $array_mg){
	...
	$member_display_inactive_acc_txt_page_title=str_replace("%total_member%","$total_member",$member_display_inactive_acc_txt_page_title);
	set_global_var("member_display_inactive_acc_txt_page_title","$member_display_inactive_acc_txt_page_title");
	
	$member_display_inactive_acc_txt_account_tobe_suspended_deleted_downgraded=str_replace("%print_mon_day_year_dropdown%","$print_mon_day_year_dropdown",$member_display_inactive_acc_txt_account_tobe_suspended_deleted_downgraded);
	set_global_var("member_display_inactive_acc_txt_account_tobe_suspended_deleted_downgraded","$member_display_inactive_acc_txt_account_tobe_suspended_deleted_downgraded");
	
	$member_display_inactive_acc_txt_select_member_group_to_move_members=str_replace("%show_member_group_all_user%","$show_member_group_all_user",$member_display_inactive_acc_txt_select_member_group_to_move_members);
	set_global_var("member_display_inactive_acc_txt_select_member_group_to_move_members","$member_display_inactive_acc_txt_select_member_group_to_move_members");
	
	set_global_var("print_object",get_html_from_layout("admin/html/admin_member_display_inactive_account.html") . $show_account_detail_table);
