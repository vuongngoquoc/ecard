...
*/
	...
	function category_menu($cat_id="",$cat_id_hilight=""){
		...
		$category_menu.='<div class="panel-group category-products" id="accordian">';
		foreach($list_data as $row_data){
			$category_menu.='<div class="panel panel-default"><div class="panel-heading"><h4 class="panel-title">';
			$val = $row_data[cat_id] ;
			...
			if($cat_id!=""){
				if($cat_id_hilight==$val){
					$category_menu.="<a class=\"active_smenu\" href=\"".print_url_cards_browse_cate($val,$row_data[cat_name_display])."\" >$cat_name_display</a>";
				}
				else{
				$category_menu.="<a href=\"".print_url_cards_browse_cate($val,$row_data[cat_name_display])."\">$cat_name_display</a>";
				}
			}
			else{
				$category_menu.="<a href=\"".print_url_cards_browse_cate($val,$row_data[cat_name_display])."\" class=\"main_category_text_link\">$cat_name_display</a>";
			}
			$category_menu.='</h4></div></div>'
		}
		$category_menu.="</div>";
	...
	function print_thumbnail($cat_id="",$no_page=""){
		...
		global $set_birthday_card,$the_template_display_thumbnail,$the_template_show_favorite,$the_template_show_thumb_toolbar,$the_template_show_cat_name_next_step_favorite;
		...
		if($count_list>0){
			...
			for ($z=0; $z<$count_list; $z++) {
				...
				if($cf_show_rate_star_image =="1"){
					...
					$show_rate_card="<img style=\"vertical-align:text-bottom\" border=\"0\" src=\"$star_rate_url\" />";
				}
				else{
					$show_rate_card ="";
				}			
	...
	function get_page_count_number($page,$b){
		...
		if ($b <10){
			for($a_num=1; $a_num<=$b; $a_num++) {
				$y++;
				if ($a_num == $page) {
					$count_number .="<li><span >$a_num</span></li>";					
				}
				else {
		...
		elseif(($page > 3) && ($page < ($b-3))){
			...
			for($a_num = $b-2; $a_num<=$b; $a_num++) {
				...
				$url_page=str_replace("%cat_name%","$title",$url_page);
				$url_page=str_replace("%keyword%","$keyword",$url_page);
				$count_number .="<a  href=\"$url_page\">$a_num</a>";
			}
		}
		else{
