...
*/
	...
	*/
	...
	$c=count($my_music);
	if($c>0){
		$show_list_music .="<div class=\"select_overflow_cell\" onclick=\"getMusic('music_mymusic')\">$sendcard_php_my_upload ($c)</div>";	
		$show_list_music.="<div style='display:none' id='music_mymusic' class='select_over_flow_cell_music'>";
		foreach($my_music as $row_data1){
				...
				if(file_exists("$ecard_root/resource/music/$row_data1[ec_cat_dir]/$row_data1[music_filename]")){
					$show_list_music .="<div class=\"select_overflow_cell\" onclick=\"cs_music_filename='$row_data1[music_filename]';GetMusicCode('resource/music/$row_data1[ec_cat_dir]/$row_data1[music_filename]','$music_name_display');\">$music_icon $row_data1[music_name_display]</div>";
				}
		}	
		$show_list_music.="</div>";
	}
