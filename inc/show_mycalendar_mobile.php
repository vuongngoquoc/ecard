...
*/
	...
	$print_drop_down_month="<select style='width:110px' class='form-control input-sm' name=\"month\" onchange=\"document.calendar_form.submit();\">\n";
	...
	$print_drop_down_year="<select style='width:80px' class='form-control input-sm' name=\"year\" onchange=\"document.calendar_form.submit();\">\n";
	...
	<div class='col-sm-1 no-padding pull-left'>
		<a class='btn btn-default btn-sm' href="$url_prv_month"><i class='fa fa-arrow-left'></i></a>
	</div>
	
	<div class='col-sm-10 pull-left  text-center h-space-b-10'>
			<form name="calendar_form" class='form-horizontal' action="index.php" method="post" >
				...
				<div class='form-inline' style='width100%'>
					$print_drop_down_month
					$print_drop_down_year
				</div>
			</form>
	</div>
	<div class='col-sm-1 no-padding pull-right text-right'>
		<a class='btn btn-default btn-sm' href="$url_next_month"><i class='fa fa-arrow-right'></i></a>
	</div>
EOF;

	//Build calendar table
	$days = Array("$Sunday", "$Monday", "$Tuesday", "$Wednesday", "$Thursday", "$Friday", "$Saturday");
	$html_calendar_table="<table class='table table-condensed table-bordered'><thead><tr class=\"active\">\n";
	foreach ($days as $day) {
		$html_calendar_table.="\t<th align=\"center\" width=\"40\">$day</th>\n";
	}
	...
	for ($count=0; $count < (6*7); $count++){
		...
		if ($count < $firstDayArray[wday] || $firstDayArray2[month] != $month) {
			...
			if ($start==$begin_today_timestamp){
				$html_calendar_table.="\t<td class=\"cell cell_today\" $cell_style title=\"$firstDayArray2[month]/$firstDayArray2[mday]/$firstDayArray2[year]\" ><strong>$firstDayArray2[mday]</strong><span class=\"cell_text_today\">$txt_calendar_today</span><div style=\"line-height:8px;\"></div>$event_holiday$event_birthday$event_anni$event_reminder</td>\n";
			}
			elseif($start < $begin_today_timestamp && $mode!="1"){
				$html_calendar_table.="\t<td class=\"cell cell_pastday\" $cell_style>$firstDayArray2[mday]<div style=\"line-height:8px;\"></div>$event_holiday$event_birthday$event_anni$event_reminder</td>\n";
			}
			else{
				$html_calendar_table.="\t<td class=\"cell cell_futureday\" $cell_style title=\"$firstDayArray2[month]/$firstDayArray2[mday]/$firstDayArray2[year]\"><a href=\"index.php?step=sign_in&next_step=show_reminder&show_add_table=1&show_time=$firstDayArray2[month]/$firstDayArray2[mday]/$firstDayArray2[year]\" class=\"day_link\">$firstDayArray2[mday]</a><div style=\"line-height:8px;\"></div>$event_holiday$event_birthday$event_anni$event_reminder</td>\n";
			}
			$start += ADAY;
	...
	$html_table .= "<div class='col-md-12'><div class='row'><div class='col-sm-12'>";
	$html_table .= "<h1 class='table_title_bar' id='title-1'><i class='fa fa-calendar padding5'></i>$txt_calendar</h1>";
	$html_table .= "$html_mon_year_table";
	$html_table .= "<div class='table-responsive cart_info'>$html_calendar_table </div>";
	$html_table .= "$html_local_time";
	...
	function get_event($emon,$eday){
		...
		foreach ($list_event as $ev_row){
			if($ev_row[event_month]==$emon && $ev_row[event_day]==$eday){
				$ev_names .= "<div id=\"ev$ev_row[event_id]\"><a href=\"$ev_row[event_url]\" class=\"event_text_link\"><i class='fa fa-globe padding5'></i> $ev_row[event_name]</a></div>";
			}
		}
	...
	function get_event_birthday($emon,$eday){
		...
		foreach ($list_birthday as $ev_row){
			if($ev_row[book_birth_mon]==$emon && $ev_row[book_birth_mday]==$eday){
				$ev_names .= "<div id=\"birthday$ev_row[book_id]\"><a href=\"$ecard_url/index.php?step=sign_in&next_step=show_addressbook&show_user_id=$ev_row[book_id]\" class=\"event_text_link\"><i title='Birthday' class='fa fa-birthday-cake padding5'></i>$ev_row[book_fname] $addressbook_txt_reminder_birthday</a></div>";
			}
		}
	...
	function get_event_anni($emon,$eday){
		...
		foreach ($list_anni as $ev_row){
			if($ev_row[book_special_mon]==$emon && $ev_row[book_special_mday]==$eday){
				$ev_names .= "<div id=\"anni$ev_row[book_id]\"><a href=\"$ecard_url/index.php?step=sign_in&next_step=show_addressbook&show_user_id=$ev_row[book_id]\" class=\"event_text_link\"><i title = 'Anniversary' class='fa fa-gift padding5'></i>$ev_row[book_fname] $addressbook_txt_reminder_anni</a></div>";
			}
		}
	...
	function get_event_reminder($emon,$eday){
		...
		foreach ($list_reminder as $ev_row){
			if($ev_row[rm_month]==$emon && $ev_row[rm_day]==$eday){
				$ev_names .= "<div id=\"anni$ev_row[rm_id]\"><a href=\"$ecard_url/index.php?step=sign_in&next_step=show_reminder&show_user_id=$ev_row[rm_id]\" class=\"event_text_link\"><i class='fa fa-clock-o padding5' title='Reminder'></i>$ev_row[rm_title]</a></div>";
			}
		}
